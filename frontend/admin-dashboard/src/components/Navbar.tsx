...
interface NavbarProps {
  pendingCount?: number; // Optional prop to show notification count if passed
}

const Navbar: FC<NavbarProps> = ({ pendingCount = 0 }) => {
  const navigate = useNavigate();

  const handleEnquiryClick = () => {
    navigate("/enquiry-management");
  };
  ...
  return (
    <header className="flex items-center justify-between px-8 h-16 bg-white shadow-sm rounded-b-xl">
      ...
      <div className="flex items-center gap-6">
        ...
        >
          {/* Show count badge only if pendingCount > 0 */}
          {pendingCount > 0 && (
            <span className="absolute top-0 right-0 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-white bg-red-600 rounded-full transform translate-x-1/2 -translate-y-1/2">
              {pendingCount}
            </span>
          )}
          {/* Small blue dot as a default indicator if no count */}
          {pendingCount === 0 && (
            <span className="inline-block w-2 h-2 bg-blue-500 rounded-full absolute top-0 right-0 transform translate-x-1/2 -translate-y-1/2"></span>
          )}
